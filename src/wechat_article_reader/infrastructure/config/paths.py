...
# 应用标识
APP_NAME = "WechatArticleReader"
APP_AUTHOR = "WechatArticleReader"
...
def get_runtime_dir() -> Path:
    ...
    项目源码运行时优先使用项目根目录下的 `.runtime`，避免沙箱或受限
    用户目录不可写；安装到其他环境时回退到平台标准数据目录。
    可通过 WECHAT_ARTICLE_READER_RUNTIME_DIR 显式覆盖。
    """
    configured = os.getenv("WECHAT_ARTICLE_READER_RUNTIME_DIR")
    if configured:
        ...
        if (project_root / "pyproject.toml").exists():
            ...
            _, _, runtime_dir = _get_platformdirs_paths()
...
def get_config_dir() -> Path:
    ...
    Windows: C:/Users/<user>/AppData/Local/WechatArticleReader
    macOS: ~/Library/Application Support/WechatArticleReader
...
def get_cache_dir() -> Path:
    ...
    Windows: C:/Users/<user>/AppData/Local/WechatArticleReader/Cache
    macOS: ~/Library/Caches/WechatArticleReader
    Linux: ~/.cache/wechatsummarizer
    """
    cache_dir = get_runtime_dir() / "cache"
...
def get_data_dir() -> Path:
    ...
    Windows: C:/Users/<user>/AppData/Local/WechatArticleReader/Data
    macOS: ~/Library/Application Support/WechatArticleReader
