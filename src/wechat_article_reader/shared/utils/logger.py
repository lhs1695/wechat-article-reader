...
def setup_logger(
    ...
    Args:
        ...
        file_enqueue: 是否通过多进程队列异步写文件；默认由环境变量控制
    """
    # 移除默认处理器
    logger.remove()
    ...
    if log_to_file:
        if log_dir is None:
            # 使用 Windows 标准路径 (AppData/Local/WechatArticleReader/logs)
            try:
                ...
                # 回退到用户目录
                log_dir = Path.home() / ".wechat_article_reader" / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)

        log_file = log_dir / LOG_FILE_NAME
        if file_enqueue is None:
            file_enqueue = os.getenv(
                "WECHAT_ARTICLE_READER_LOG_ENQUEUE", "false"
            ).strip().lower() in {
                "1",
                "true",
                "yes",
        ...
        try:
            logger.add(log_file, enqueue=file_enqueue, **file_handler_options)  # type: ignore[call-overload]
        except PermissionError:
            if not file_enqueue:
                raise
            logger.add(log_file, enqueue=False, **file_handler_options)  # type: ignore[call-overload]
            logger.warning("日志队列不可用，已降级为同步文件写入")
...
def log_event(
    ...
    **kwargs: Any,
    ...
    Examples:
        ...
        log_event("summary_generated", model="deepseek", tokens=500)
    """
    # 添加 request_id
