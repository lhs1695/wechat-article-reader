...
class ErrorCode(Enum):
    ...
    SCRAPER_RATE_LIMITED = (2006, "请求被限流")
...
class WechatArticleReaderError(Exception):
    """基础异常类
...
class DomainError(WechatArticleReaderError):
    """领域异常基类"""
...
class ApplicationError(WechatArticleReaderError):
    """应用层异常基类"""
...
class InfrastructureError(WechatArticleReaderError):
    """基础设施异常基类"""
...
class ScraperRateLimitedError(ScraperError):
    ...
    error_code = ErrorCode.SCRAPER_RATE_LIMITED
...
def wrap_exception(
    exc: Exception,
    error_class: type[WechatArticleReaderError] = WechatArticleReaderError,
    message: str | None = None,
) -> WechatArticleReaderError:
    """将普通异常包装为 WechatArticleReaderError
...
def log_errors(
    *,
    ...
    示例：
        @log_errors(context="export_document")
        def export(path):
            ...

    Args:
        reraise: 记录后是否重新抛出异常。True 则抛出，False 则返回 default_return。
        default_return: reraise=False 时的回退返回值。
        context: 日志消息中的上下文标识。
    """
