...
from wechat_article_reader.application.use_cases.export_article import ExportArticleUseCase
from wechat_article_reader.domain.entities import Article
from wechat_article_reader.shared.exceptions import UseCaseError
...
class TestExportArticleUseCase:
    ...
    def mock_unavailable_exporter(self) -> Mock:
        exporter = Mock()
        exporter.name = "unavailable"
        exporter.target = "unavailable"
    ...
    def use_case(
        ...
        return ExportArticleUseCase(
            exporters={
                ...
                "unavailable": mock_unavailable_exporter,
            }
        )
    ...
    def test_export_unknown_target_raises(
        ...
        with pytest.raises(UseCaseError, match="未找到导出目标"):
            use_case.execute(sample_article, "missing")
    ...
    def test_export_unavailable_target_raises(
        ...
        with pytest.raises(UseCaseError, match="不可用"):
            use_case.execute(sample_article, "unavailable")
