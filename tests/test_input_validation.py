...
from wechat_article_reader.domain.value_objects.content import ArticleContent
from wechat_article_reader.domain.value_objects.url import ArticleURL
from wechat_article_reader.shared.exceptions import InvalidURLError
...
class TestFilenameSanitization:
    ...
    def test_removes_windows_reserved_chars(self):
        """移除Windows保留字符"""
        from wechat_article_reader.infrastructure.adapters.exporters.markdown import (
            MarkdownExporter,
        )
        ...
        # 创建一个带有特殊字符的文章
        article = Article(
            url=ArticleURL.from_string("https://mp.weixin.qq.com/s/filename-a"),
            title='file<>:"|?*name',
            content=ArticleContent.from_text("test"),
        )
    ...
    def test_handles_very_long_filename(self):
        ...
        # 创建一个超长标题的文章
        article = Article(
            url=ArticleURL.from_string("https://mp.weixin.qq.com/s/filename-b"),
            title="a" * 300,
            content=ArticleContent.from_text("test"),
        )
    ...
    def test_preserves_chinese_characters(self):
        ...
        article = Article(
            url=ArticleURL.from_string("https://mp.weixin.qq.com/s/filename-c"),
            title="这是一篇中文文章标题",
            content=ArticleContent.from_text("test"),
        )
