...
from wechat_article_reader.domain.entities import Article, Summary
from wechat_article_reader.domain.value_objects import ArticleContent, ArticleURL
from wechat_article_reader.infrastructure.config.container import Container
from wechat_article_reader.shared.utils import utc_now
...
def _isolate_test(monkeypatch: pytest.MonkeyPatch) -> Generator[None]:
    ...
    """
    from wechat_article_reader.infrastructure.config.container import reset_container
    from wechat_article_reader.infrastructure.config.settings import reset_settings
    from wechat_article_reader.mcp.security import reset_security_manager
    ...
    if current_item is None or "integration" not in [m.name for m in current_item.iter_markers()]:
        ...
        monkeypatch.setattr(
            "wechat_article_reader.infrastructure.config.container._container",
            _minimal,
        )
        # 防止测试误触外部网络
...
def sample_summary() -> Summary:
    """示例摘要实体"""
    return Summary(
        overview="这是一篇关于人工智能如何改变生活的测试文章摘要。",
        key_points=(
            "人工智能正在改变生活方式",
            "文章包含多个测试段落",
        ),
