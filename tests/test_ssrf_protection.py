...
from wechat_article_reader.shared.utils.ssrf_protection import (
...
class TestSafeFetchRedirects:
    ...
    async def test_resolves_relative_redirect_before_validation(self):
        ...
        with (
            patch(
                "wechat_article_reader.shared.utils.ssrf_protection.create_safe_async_client",
                return_value=client,
            ),
            patch(
                "wechat_article_reader.shared.utils.ssrf_protection.SSRFSafeTransport.validate_url",
...
class TestSafeFetchProxyHandling:
    ...
    def test_safe_fetch_sync_rejects_proxy(self):
        """不可信 URL 抓取不能切换到绕过安全 transport 的代理。"""
        with pytest.raises(SSRFBlockedError, match="Proxy use is disabled"):
            safe_fetch_sync(
                "https://example.com",
    ...
    async def test_safe_fetch_does_not_forward_none_proxy_to_async_request(self):
        ...
        with (
            ...
            response = await safe_fetch("https://example.com", proxy=None)

        assert response.text == "ok"
        mock_factory.assert_called_once_with()


class TestSafeFetchResponseLimits:
    @pytest.mark.asyncio
    async def test_streaming_limit_rejects_oversized_body(self):
        async def handler(_request: httpx.Request) -> httpx.Response:
        ...
        with (
            ...
            patch.object(SSRFSafeTransport, "validate_url", return_value="https://example.com"),
    ...
    def test_rejects_non_html_success_response(self):
        ...
        with (
            patch(
                "wechat_article_reader.shared.utils.ssrf_protection.create_safe_client",
                return_value=client,
            ),
            patch.object(SSRFSafeSyncTransport, "validate_url", return_value="https://example.com"),
            pytest.raises(UnsafeContentTypeError),
        ):
            safe_fetch_sync(
                "https://example.com",
